...
int main(){
    ...
    while(choice != 0 && cin.good()){
        ...
        std::cout<<" 1. [1] Test Reservation overlaps function"<<endl;
        std::cout<<" 2. [1] Test isMatch function"<<endl;
        std::cout<<" 3. [1] Test add room function"<<endl;
        std::cout<<" 4. [1] Test add room for numeric order"<<endl;
        std::cout<<" 5. [1] Test delete room"<<endl;
        std::cout<<" 6. [2] Test add reservation to room"<<endl;
        std::cout<<" 7. [1] Test add reservation to room for correct order"<<endl;
        std::cout<<" 8. [3] Test add reservation to Hotel"<<endl;
        std::cout<<" 9. [Total: 11] Test all and get number of Tests Passed"<<endl<<endl;
        cin >> choice;
        std::cout<<endl;
        switch (choice)
        {
        ...
        case 9:
            ...
            std::cout << "Tests Passed: "<<score<<" out of 11"<<endl<<endl;   
            break;
        }
    }
    std::cout<<"Bye!"<<endl;
...
int testResOverlap(){
    ...
    // 1. testing cross month overlap, should overlap
    Date date9(2018, 10, 29);
    Date date10(2018, 11, 2);
    Reservation r00("Emily", date9, 10);
    Reservation r11("Sam", date10, 3);

    std::cout<<endl;
    if (!resOverlap(r00, r11, true)) score = 0;

    // 2. Same leave date and start date, should not overlap
    Date date11(2020, 4, 2);
    Date date12(202, 4, 4);
    Reservation r12("Joe", date11, 2);
    Reservation r13("Don", date12, 6);

    std::cout<<endl;
    if (!resOverlap(r12, r13, false)) score = 0;

    // 3. Long Duration Overlap, should overlap
    Date date13(2021, 7, 1);
    Date date14(2021, 8, 3);
    Reservation r14("Dave", date13, 35);
    Reservation r15("Joe", date14, 5);

    std::cout<<endl;
    if (!resOverlap(r14, r15, true)) score = 0;

    // 4. All Different values & long durations, should not overlap
    Date date15(2000, 8, 12);
    Date date16(2023, 1, 2);
    Reservation r16("Quin", date15, 100);
    Reservation r17("Joe", date16, 57);

    std::cout<<endl;
    if (!resOverlap(r16, r17, false)) score = 0;
...
int testIsMatch(){
    ...
    std::cout << "room isMatch all tests passed, 1 point"<<endl;
    cout<<"Score: "<<SCORE<<endl;
    return SCORE;
}
...
int testAddRoom(){
    ...
    std::cout<<"All room numbers found, 1 point"<<endl;
    return SCORE;
}
...
int testAddRoomOrder(){
    ...
    std::cout<<"All room found in order, 1 point"<<endl;
    return SCORE;
}
...
int testDeleteRoom(){
    ...
    std::cout<<"deleteRoom success, "<<SCORE<<" point"<<endl;
    return SCORE;
...
int testAddResRoom(){
    ...
    std::cout<<"All names found, 1 point"<<endl<<endl;
    score += SCORE;
    ...
    std::cout<<names[NUM_NAMES-1]<<" reservation not found, test passed, 1 point"<<endl;
    std::cout<<"Total points: "<<score + SCORE<<" points"<<endl;
...
int testAddResRoomOrder(){
    ...
    std::cout<<"All names found in order, 1 point"<<endl<<endl;
    return SCORE;    
}
...
int testAddRes(){
    ...
    //replace std::out 
    std::cout.rdbuf( oldCoutStreamBuf );
    std::cout<<"All names found 3 points"<<endl;
    return 3;
}